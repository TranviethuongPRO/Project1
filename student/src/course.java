 ...
 */
public class course {
     public void insertUpdateDeleteStudent(char operation, Integer id, String subject, Integer hours) //String lname,String sex,
    // String bdate, String phone, String address)
    {
        ...
        if (operation == 'i') {//for insert
            try {
                ps = con.prepareStatement("INSERT INTO course (subject, hours_number) VALUES (?,?)");
        ...
        if (operation == 'u') {//for update
            try {
                ps = con.prepareStatement("UPDATE `course` SET `subject`= ?, `hours_number` = ? WHERE `id` = ?");
    ...
    PreparedStatement ps;
       ...
       public void fillCourseJtable(JComboBox combo){
    Connection con = MyConnection.getConnection();
    PreparedStatement ps;
        try {
